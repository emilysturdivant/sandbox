...
facet_ncol <- 2
p <- loss_df %>% 
  mutate(loss_fc_ha = loss_fc_ha) %>% 
  ggplot(aes(x=Year, y=loss_fc_ha)) +
  geom_line(show.legend = FALSE, color='#F5A011') +
  geom_area(alpha = 0.7, show.legend=FALSE, fill='#F5A011') + 
  # geom_bar(stat='identity', alpha = 0.7, fill='#F5A011', show.legend=FALSE) + 
  scale_y_continuous(labels=label_number(scale_cut=cut_short_scale()), 
                     name='Forest area lost (ha)') + 
  scale_x_continuous(labels=label_yearintervals, n.breaks=6, name='Year interval') + 
  coord_cartesian(xlim=c(2001, 2022), expand=T) +
  facet_wrap(vars(div_name), 
             scales='fixed',
             ncol=facet_ncol) +
  # cowplot::theme_minimal_grid() + #cowplot::background_grid() +
  theme_bw() +
...
loss_17to22_df <- df %>% 
  group_by(div_name) %>% 
  arrange(stock_year) %>% 
  mutate(cum_loss_fc = cumsum(loss_fc_ha),
         fc_area = fc_area_2000 - cum_loss_fc) %>% 
  filter(stock_year > lastyr-5 & stock_year <= lastyr) %>% 
  summarize(loss_fc_ha = mean(loss_fc_ha, na.rm=T), 
            fc_area_2000 = mean(fc_area_2000, na.rm=T), 
            fc_area_ly = last(fc_area), 
            cum_loss_fc = last(cum_loss_fc)) %>% 
  select(div_name, 
...
png_fp <- here::here(out_dir, paste0('table_fc_area_', endyr, '.png'))
