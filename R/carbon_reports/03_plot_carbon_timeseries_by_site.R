...
p_stock <- p_stock_full +
  ...
  theme(
    ...
    legend.position = params2$legend.position,
...
# Save with zoom lines
png(here::here(out_dir, str_c(params2$location, '_carbon_stock_2panel.png')),
    width = 7.2, height = 4, units = 'in', res = 150)
p_out
grid.draw(linesGrob(x = unit(c(0.242, 0.34), "npc"), 
                    y = unit(c(0.96, 0.96), "npc"),
                    gp = gpar(lty = 2, lwd = 1)))
grid.draw(linesGrob(x = unit(c(0.242, 0.34), "npc"), 
                    y = unit(c(0.76, 0.15), "npc"),
                    gp = gpar(lty = 2, lwd = 1)))
dev.off()

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Stock + annual change bars: 2 panels ----
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
colr_loss_pattern = '#aa2143'
colr_stock_area <- '#9bdeac'
colr_stock_line <- "seagreen4"
...
p_bar <- df_annual %>%
  ...
  geom_bar(position="dodge", stat="identity", alpha = 0.7,
           width = 280) +
  scale_color_manual(name = '',
                    values = c(Gain = colr_stock_line,
                               Loss = colr_loss_pattern),
                    labels = c(Gain = "Annual growth in C", 
...
# Save in 3-panel arrangement with zoom lines
# Combine in patchwork
(p2_stock <- p_stock_full + plot_spacer() + p_stock + 
    plot_layout(ncol = 3, widths = c(1, 0.2, 4)))

(p_out <- p2_stock / 
    p_bar + plot_layout(nrow = 2, heights = c(2,1)))

# Save with zoom lines
png(here::here(out_dir, str_c(params2$location, '_carbon_stockbar_3panel.png')),
    width = 7.2, height = 7, units = 'in', res = 150)
p_out
grid.draw(linesGrob(x = unit(c(0.243, 0.355), "npc"),
                    y = unit(c(0.977, 0.977), "npc"),
                    gp = gpar(lty = 2, lwd = 1)))
grid.draw(linesGrob(x = unit(c(0.243, 0.355), "npc"),
                    y = unit(c(0.875, 0.475), "npc"),
                    gp = gpar(lty = 2, lwd = 1)))
dev.off()

# Save as 2-panel without full view
(p_out <- (p_stock ) / 
    p_bar + plot_layout(nrow = 2, heights = c(1,1)) & 
    theme(plot.margin = margin(0,1,0,1, "mm")))

ggsave(here::here(out_dir, str_c(params2$location, '_carbon_bar2panel.png')), 
       plot = p_out,
       width = 7.2, # 4.5
       height = 8, # 2.9
       dpi = 150)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Two panels: stock and annual change lines ----
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
colr_nochange_area = '#9bdeac'
colr_gain_pattern = 'seagreen4'
colr_loss_pattern = '#aa2143'
...
# Line chart of gain and loss
p_line <- df_annual %>% 
  mutate(Loss = Loss * -1) %>% 
  select(year, Gain, Loss) %>% 
  pivot_longer(cols = Gain:Loss) %>% 
  ggplot(aes(x = year, y = value, color = name)) +
  geom_point(position=position_dodge(width = NULL), stat="identity", size = 2) +
  geom_line(position=position_dodge(width = NULL), stat="identity", size = 1) +
  scale_color_manual(name = '',
                    values = c(Gain = colr_stock_line,
                               Loss = colr_loss_pattern),
                    guide='none'
  ...
  scale_y_continuous(name = paste0('Change in carbon (', units.short, ')'),
                     # labels = scales::label_comma(accuracy = 0.01),
                     labels = scales::label_comma(accuracy = 0.01),
                     breaks = scales::breaks_extended(n = 7),
                     # limits = c(min = bar_ymin - bar_ypad, max = bar_ymax + bar_ypad),
...
(p_out <- (p_stock_full + p_stock + plot_layout(ncol = 2, widths = c(1, 4))) / 
    p_line + plot_layout(nrow = 2, heights = c(2,1)))

ggsave(here::here(out_dir, str_c(params2$location, '_carbon_line3panel.png')), 
       plot = p_out,
       width = 7.2, # 4.5
       height = 8, # 2.9
       dpi = 150)
