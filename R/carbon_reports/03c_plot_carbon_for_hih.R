...
final_polys_dir <- '~/data/hih_sites'
polys_fp <- here::here('~/data/hih_sites/HIH_RX_sites_feb2023.geojson')
...
site_name_var <- 'site_name' # Estonia: 'name'
site_div_var <- 'div_name' # Estonia: 'div1'
div_column_name <- 'div_name'
...
df_sf <- st_read(out_fp) %>% 
  bind_cols({{site_div_var}} := sf_in[[site_div_var]]) 
# |> 
...
df_sf_sums <- df_sf %>% 
  ...
  dplyr::select(any_of(c(site_div_var, 'name')), 
         area_ha,
         forest_2000_ha, 
         forest_loss_ha,
         forest_loss_pct,
...
if (round(max(df_site$annual_loss_tC)/1e6) > 1) {
  df_site_t <- df_site %>% 
    mutate(c_loss_flex = annual_loss_tC / 1e6)
  
  c_units = 'MtC'
} else {
  df_site_t <- df_site %>% 
    mutate(c_loss_flex = annual_loss_tC)
  c_units = 'tC'
}
...
df_p %>% 
  mutate(annual_loss_tC = annual_loss_tC * -1) %>% 
  write_csv(here::here('outputs', site, str_c('agb30m_hansen_loss_y01_21.csv')))
...
df_p |> 
  filter(.data[[site_div_var]] == 'Gunung Naning' | .data[[site_div_var]] == 'Gunung Nyiut' ) %>% 
