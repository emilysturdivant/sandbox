...
```{r extract-tropics-flii}
# FLII
out_dir <- file.path(data_dir, 'forests', 'flii_tropics')
(flii_tifs <- list.files(file.path(data_dir, 'forests', 'flii'), 
                       pattern = 'flii_[^earth].*\\.tif$', 
                       full.names = TRUE))

agg_and_crop <- function(fp) {
  # Get output name
  ...
  # Get intersection of the bounding boxes of the two rasters (didn't work with terra::intersect)
  ...
  # Aggregate by factor of 3
  r_agg <- terra::aggregate(r, 3)
  
  # Crop and save
  terra::crop(r_agg, bbex, filename = fn_ac, overwrite = TRUE)
}

flii_tifs %>% purrr::walk(agg_and_crop)
...
```{r extract-tropics-hm}
# FLII
in_dir <- file.path(data_dir, 'human_influence', 'gHM')
out_dir <- in_dir
(tifs <- list.files(in_dir, pattern = '.tif$', full.names = TRUE))
fp <- tifs[[1]]
